...
class C3UCBBaseBandit:
    ...
    @abstractmethod
    def load_params(self, params_load):
        pass

    @abstractmethod
    def save_params(self, params_save):
        pass
...
class C3UCB(C3UCBBaseBandit):
    def load_params(self, params_load):
        params = torch.load(params_load)

        self.hyper_alpha = params["hyper_alpha"]
        self.v = params["v"]
        self.b = params["b"]

        logging.info(f"Load the params value from `{params_load}`.")

    def save_params(self, params_save):
        params = {"hyper_alpha": self.hyper_alpha, "v": self.v, "b": self.b}
        torch.save(params, params_save)

        logging.info(f"Dump the params value into `{params_save}`.")

    def select_arm(self, context_vectors, current_round):
        pass

    def select_arm_v2(self, context_vectors, is_final):
        """
        ...
        logging.info(f"================================\n"
                     f"weight_vector: {weight_vector.transpose().tolist()[0]}")
        ...
        return self.oracle.get_super_arm(self.upper_bounds, self.context_vectors, self.arms, is_final)
