class BanditArm:
    def __init__(self, index_cols, table_name, memory, table_row_count, include_cols=()):
        ...
        if self.include_cols:
            ...
            include_col_names = '_'.join(tuple(map(lambda x: x[0:4], include_cols)))
            self.index_name = 'IXN_' + table_name + '_' + '_'.join(index_cols) + '_' + include_col_names
        else:
            self.index_name = 'IX_' + table_name + '_' + '_'.join(index_cols)
