...
from index_advisor_selector.index_selection.heu_selection.heu_utils.heu_com import get_utilized_indexes, mb_to_b, b_to_mb
...
# This algorithm resembles the index selection algorithm published in 2000 by Valentin et al.
...
class DB2AdvisAlgorithm(SelectionAlgorithm):
    ...
    def _calculate_best_indexes(self, workload, db_conf=None, columns=None):
        ...
        #         self.step[self.layer].append({"combination": item.index,
    ...
    def _calculate_index_benefits(self, candidates, query_results):
        ...
        for index_candidate in candidates:
            ...
            for query, value in query_results.items():
                ...
                #     benefit += -1 * value["cost_with_indexes"] * b_to_mb(index_candidate.estimated_size)
                # elif self.sel_oracle == "cost_pure":
                #     benefit += -1 * value["cost_with_indexes"]
                # elif self.sel_oracle == "benefit_per_sto":
                #     benefit += (value["cost_without_indexes"] - value["cost_with_indexes"]) / b_to_mb(index_candidate.estimated_size)
                # elif self.sel_oracle == "benefit_pure":
                #     benefit += value["cost_without_indexes"] - value["cost_with_indexes"]

            indexes_benefit.append(IndexBenefit(index_candidate, benefit, self.sel_oracle))
    ...
    def _combine_subsumed(self, index_benefits):
        ...
        if self.sel_oracle == "benefit_pure":
            assert not (index_benefits == sorted(
                index_benefits,
                reverse=True,
...
class IndexBenefit:
    ...
    def __lt__(self, other):
        ...
        if self.sel_oracle is None:
            ...
            self_ratio = self.benefit
            other_ratio = other.benefit
