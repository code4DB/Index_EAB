...
def which_queries_to_remove(plans, queries_to_remove, seed=None,
                            excluded_query_classes=frozenset([])):
    """
    Determine the total number of `queries_to_remove` queries to remove that:
    1) do not influence the number of operators;
    2) have less impact on the reduction of the number of operators (more reserved).
    :param plans:
    :param queries_to_remove:
    :param seed:
    :param excluded_query_classes:
    :return:
    """
    random.seed(seed)
    ...
    # [query plan `without` indexes], [query plan `with` indexes]
    for plan in plans[0]:
